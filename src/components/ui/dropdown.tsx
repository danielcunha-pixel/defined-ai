...
function Dropdown({
  ...
  const displayText = isSelected ? selectedLabel : placeholder;
  ...
  const triggerClasses = cn(
    ...
    style === "inline" && (isSelected ? "w-[148px]" : "w-[222px]"),
    ...
    >
      <span
        className={cn(
          ...
          triggerTypography,
          isDisabled ? "text-grey-30" : isSelected ? "text-grey-100" : "text-grey-60"
        )}
      >
        {displayText}
      </span>
  ...
  if (style === "inline") {
    return (
      ...
      >
        {label && <span id={labelId} className="pb-sp-2 text-[15px] font-medium leading-normal text-grey-100">{label}</span>}
  ...
  return (
    ...
    >
      {label && (
        <span id={labelId} className="text-[15px] font-medium leading-normal text-grey-100">
          {label}
        </span>
      )}
      ...
      {helperText && (
        <p className={cn("text-[13px] font-normal leading-normal tracking-[0.04em]", isDisabled ? "text-grey-40" : "text-grey-70")}>
          {helperText}
        </p>
      )}
    </div>
